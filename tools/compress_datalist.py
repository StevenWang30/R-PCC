...
parser.add_argument('--datalist', help='datalist for batch of point cloud compression. Example datalist in ./data/*.txt')
...
def compress():
    ...
    dataset = build_dataset(datalist=args.datalist, lidar_type=args.lidar)
    ...
    def compress_dataset(index):
        t_init = time.time()
        point_cloud, range_image, original_point_cloud, file_name = dataset[index]
        ...
        output_path = output_path.replace(output_path.split('.')[-1], 'rpcc')
        os.makedirs(os.path.dirname(output_path), exist_ok=True)
        save_compressed_bitstream(output_path, compressed_data, uniform=uniform)
        t_save = time.time()

        if args.output:
            print('\nCompression finished.')
            print('binary bitstream save in ', output_path)
            ...
            compressed_bit_size = os.path.getsize(output_path) * 8
            print('\nCompression Results: ')
            ...
            if args.eval:
                # reconstruct
                compressed_data = read_compressed_bitstream(output_path, uniform=uniform)
    ...
    with futures.ThreadPoolExecutor(args.workers) as executor:
        if args.output:
            executor.map(compress_dataset, np.arange(len(dataset)))
        else:
            list(tqdm(executor.map(compress_dataset, np.arange(len(dataset))), total=len(dataset)))
