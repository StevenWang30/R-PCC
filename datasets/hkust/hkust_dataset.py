...
class HkustCampusDataset(DatasetTemplate):
    def __init__(self, datalist=None, dataset_cfg=None, channel_distribute_csv=None, use_radius_outlier_removal=False):
        super(HkustCampusDataset, self).__init__(datalist, dataset_cfg, channel_distribute_csv, use_radius_outlier_removal)
        print('dataset initialize finished.')

    def preprocess_pcd_to_bin(self, data_root='/data/HKUST_campus_VLP16/point_clouds'):
        ...
        for dir in file_dir:
            file_list = glob.glob(os.path.join(dir, 'velodyne_points/data/*.pcd'))
            file_list.sort()
            for (i, file) in enumerate(file_list):
                save_path = file.replace('velodyne_points/data', 'velodyne_points/data_bin')
...
if __name__ == '__main__':
    ...
    data_list = '../../data/test_16E_HKUST_campus.txt'
    dataset_cfg = './cfg/Velodyne_VLP_16E.yaml'
    dataset = HkustCampusDataset(data_list, dataset_cfg)
